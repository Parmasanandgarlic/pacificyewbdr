...
from bdr_agent import discover_businesses, qualify_and_draft, get_supabase
...
def extract_email_draft(analysis_text: str) -> str:
    """Try to pull just the email portion from the agent's full output."""
    if not analysis_text:
        return ""

    # Look for common email markers
    patterns = [
        r"(?:Subject:.*?\n)([\s\S]+)",                       # After Subject: line
        r"(?:Email Draft:?|Draft:?|Cold Email:?)\s*\n([\s\S]+)",  # After "Draft:" header
        r"(?:Dear|Hi|Hello)\s+[\w\s,]+[\s\S]+",              # Starts with greeting
    ]
    ...
    # Fallback: return the full text if we can't isolate the email
    return analysis_text


def get_status_badge(status: str) -> str:
    """Return an HTML badge for the lead status."""
    css_class = {
...
all_leads = leads_response.data if leads_response.data else []
...
with col_m1:
    ...
    <div class="metric-card">
        <div class="metric-value">{len(all_leads)}</div>
...
with btn_col1:
    ...
    if st.button("✅ Approve & Send", use_container_width=True, key="btn_approve"):
        sb.table("leads").update({"status": "APPROVED"}).eq("id", lead["id"]).execute()
        st.toast("Lead approved! 🎉", icon="✅")
        st.rerun()
...
with btn_col2:
    ...
    if st.button("❌ Reject", use_container_width=True, key="btn_reject"):
        sb.table("leads").update({"status": "REJECTED"}).eq("id", lead["id"]).execute()
...
with btn_col3:
    if st.button("💾 Save Edits", use_container_width=True, key="btn_save"):
        sb.table("leads").update({"agent_analysis": edited_draft}).eq("id", lead["id"]).execute()
